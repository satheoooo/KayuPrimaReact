    ...
    <>
      <section className="px-4 md:px-8 lg:px-[160px] py-12 md:py-16 lg:py-[80px] bg-white">
        ...
        <div className="relative">
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 gap-5 mb-6 ${shouldBlur ? "blur-md pointer-events-none" : ""}`}>
            {premiumBuyers.map((buyer) => (
              <div
                key={buyer.id}
                className="relative bg-gradient-to-br from-[#2F5E2F] to-[#1a3d1a] rounded-2xl p-6 text-white overflow-hidden shadow-[0_8px_30px_-8px_rgba(47,94,47,0.4)]"
              >
                {/* Crown Icon */}
                ...
                {/* Premium Badge */}
                <div className="inline-flex items-center gap-1.5 bg-white/15 backdrop-blur-sm px-3 py-1 rounded-full mb-4">
                  <svg className="w-3 h-3 text-yellow-400" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M5 16L3 5l5.5 5L12 4l3.5 6L21 5l-2 11H5zm14 3c0 .6-.4 1-1 1H6c-.6 0-1-.4-1-1v-1h14v1z" />
                  </svg>
                  <span className="text-[11px] font-bold tracking-wider">PREMIUM</span>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-[16px] font-bold mb-1">{buyer.name}</h3>
                    <div className="flex items-center gap-1 text-[12px] text-white/60 mb-3">
                    ...
                    <div className="flex items-center gap-5 mb-4">
                      {[
                        { label: "Butuh", value: buyer.woodNeeded },
                        { label: "Jumlah", value: buyer.quantity },
                        { label: "Budget", value: buyer.budget },
                      ].map((item, i) => (
                        <div key={i}>
                          <p className="text-[10px] text-white/40 uppercase tracking-wider">{item.label}</p>
                          <p className="text-[13px] font-bold">{item.value}</p>
                        </div>
                      ))}
                    </div>

                    <button className="w-full py-2.5 bg-white text-[#2F5E2F] rounded-xl text-[13px] font-medium hover:bg-white/90 active:scale-[0.98] transition-all duration-150 flex items-center justify-center gap-2">
          ...
          {shouldBlur && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center bg-white rounded-2xl p-8 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] border border-gray-100/60">
                ...
                <p className="text-[13px] text-gray-400 mb-5 max-w-[280px]">
                  {!isAuthenticated
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-[#2F5E2F] text-white rounded-xl text-[13px] font-medium hover:bg-[#244824] active:scale-[0.98] transition-all duration-150"
                >
          ...
          {/* View All Link */}
          <div className={`text-center mt-8 ${shouldBlur ? "blur-md pointer-events-none" : ""}`}>
            <button
              onClick={handleViewAllClick}
              className="inline-flex items-center gap-2 text-[#2F5E2F] text-[14px] font-medium hover:gap-3 transition-all duration-200"
            >
              Lihat Selengkapnya
